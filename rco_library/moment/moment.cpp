...
inline const double Moment::initial_As_req() {
...
inline const double Moment::a(const double& As) {
...
inline double phi_moment(Moment m, const double& As_, double& eps_t_) {
	double eps_ty{};
	if (m.fy == 420)
		eps_ty = double(0.002);
	else
...
double phiMn(
	...
	std::cout << "===========================================================================\n";
	std::cout << "Starting the iteration to compute nominal moment capacity of concrete beam!\n";
	std::cout << "===========================================================================\n";
	std::cout << "initial As longitudinal required : " << m.initial_As_req() << " mm2\n";

	//compute number of bar reinforcement required
	double n_ = m.initial_As_req() / (double(0.25) * M_PI * dia_long * dia_long);
	std::cout << "initial number of longitudinal bars required : " << n_ << " bars\n";
	if(n_ > 0.)
		std::cout << "initial number of longitudinal bars required(roundup) : " << std::ceil(n_) << " bars\n";
	
	std::cout << "initial number of bars provided : " << *nb_bar << " bars\n";

	if (*nb_bar >= n_) {
		std::cout << "if initial number of bars provided : " << *nb_bar << " bars > initial number of bars required : " << n_ << " bars\n";
		std::cout << "assumption, initial number of bars required is using number of bars provided : " << *nb_bar << " bars\n";
		n_ = *nb_bar;
	}
	else if (n_ <= 1) {
		std::cout << "if initial number of bars required : " << n_ << " bars <= 1\n";
		std::cout << "initial number of bars required used : " << 2.0 << " bars\n";
	...
	unsigned int i = 1;
	do {
		std::cout << "iteration_" << i << '\n';
		std::cout << "n longitudinal required from the iteration : " << n_ << " bars\n";
		std::cout << "n longitudinal provided from the iteration : " << std::ceil(n_) << " bars\n";
		...
		*As_required = As_req_iteration;
		...
		std::cout << "As longitudinal required from the iteration : " << As_req_iteration << " mm2\n";
		std::cout << "As longitudinal provided from the iteration : " << As_prov << " mm2\n";
		...
		n_dummy = n_dummy + 1.0;
		std::cout << "maximum number of bars per layer from the calculation : " << n_dummy << " bars per layer\n";
		std::cout << "maximum number of bars per layer recommended : " << std::trunc(n_dummy) << " bars per layer\n";
...
*/
		...
		std::cout << "number of bars, layer-1 : " << n1 << " bars" <<
...
//			"\nnumber of bars, layer-4 : " << n4 << " bars\n";
		...
		std::cout << "minimum clear spacing bar : " << clear_spacing_long << " mm\n";
		std::cout << "maximum bar spacing : " << max_bar_spacing << " mm\n";
		...
		std::cout << "clear spacing bar provided : " << clear_spacing_provided << " mm\n";
...
//		phiMn = phi_ * (As1 * m.fy * d1 + As2 * m.fy * d2 + As3 * m.fy * d3 + As4 * m.fy * d4 - (As1 + As2 + As3 + As4) * m.fy * a_ / 2.0) / 1000000.0; //kN.m
		...
		std::cout << "minimum and maximum reinforcement ratio:\n";
		As_min1 = 0.25 * std::pow(m.fc, 0.5) / m.fy * m.b * m.d;
		As_min2 = 1.4 / m.fy * m.b * m.d;
		As_minimum = std::max(As_min1, As_min2);
		std::cout << "As minimum requirement 1 [0.25 sqrt(fc')/fy *bd] : " << As_min1 << " mm2\n";
		std::cout << "As minimum requirement 2 [1.4/fy *bd] : " << As_min2 << " mm2\n";
		std::cout << "As minimum requirement [used] : " << As_minimum << " mm2\n";
		...
		std::cout << std::fixed << std::setprecision(4);
		std::cout << "epsilon t for computing rho maximum: " << eps_t << '\n';
		std::cout << "rho maximum per rho balance : " << rho_max_per_rho_balance << '\n';
		std::cout << "rho maximum : " << rho_maximum << '\n';
		As_maximum = rho_maximum * m.b * m.d;
		std::cout << std::fixed << std::setprecision(2);
		std::cout << "As maximum requirement : " << As_maximum << " mm2\n";

		//error handling
		try {
			if (As_prov >= As_minimum && As_prov <= As_maximum) {
				std::cout << "Ok, As minimum : " << As_minimum << " mm2 < " << "As longitudinal reinforcement provided : " << As_prov << " mm2 < " << "As maximum : " << As_maximum << " mm2\n";
			}
			else if (As_prov < As_minimum && phiMn >= 4.0 / 3.0 * m.Mu) {
				std::cout << "As longitudinal reinforcement provided : " << As_prov << " < As minimum : " << As_minimum << " and phiMn : " << phiMn << " kN.m > (4/3) Mu : " << 4. / 3 * m.Mu << " kN.m\n";
				std::cout << "Ok, the element structure is big and massive\n";
			}
			else if (As_prov < As_minimum)
			{
				std::cout << "As_provided : " << As_prov << " mm2 < As_minimum : " << As_minimum << " mm2\n";
				std::cout << "reinforcement ratio provided is lesser then minimum reinforcement ratio\n";
			}
		...
		if (phi_ == 0.9)
			std::cout << "the design assumption of using phi is 0.9 in tension-controlled is verified, is phiMn > Mu ?\n";
		else {
			std::cout << "the phi calculation is " << phi_ << " , is phiMn > Mu ?\n";
		}
		
		//check is phi Mn > Mu and As provided?
		if (phiMn >= 4./3 * m.Mu && As_prov < As_minimum) {
			std::cout << "whereas As_provided: " << As_prov << " mm2 << As_minimum: " << As_minimum << " mm2\n";
			std::cout << "but it's Ok, since phiMn: " << phiMn << " kN.m > 4/3 Mu: " << 4./3 * m.Mu << " kN.m ";
			std::cout << "[massive structure]\n";
		}
		else if (phiMn >= m.Mu && As_prov >= As_minimum) {
			std::cout << "Ok, phiMn: " << phiMn << " kN.m > Mu: " << m.Mu << " kN.m ";
			std::cout << "and As_provided: " << As_prov << " mm2 > As_minimum: " << As_minimum << " mm2\n";
		}
		else if (phiMn < m.Mu && As_prov > As_minimum) {
			std::cout << "As_provided: " << As_prov << " mm2 > As_minimum: " << As_minimum << " mm2\n";
			std::cout << "but phiMn: " << phiMn << " kN.m < Mu: " << m.Mu << " kN.m, re-calculate again!\n";
		}
		else {
			std::cout << "phiMn: " << phiMn << " kN.m > Mu: " << m.Mu << " kN.m\n";
			std::cout << "but As_provided: " << As_prov << " mm2 < As_minimum: " << As_minimum << " mm2, re-calculate again!\n";
		}
	...
	std::cout << "calculation of the nominal moment capacity is complete!\n";
	std::cout << "===========================================================================\n";
	return phiMn;
}
